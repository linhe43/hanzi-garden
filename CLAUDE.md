...
- **Bump the cache.** Any change to a cached file (`index.html`, `data/charsets.json`, `fonts/kai.woff2`, `privacy.html`, manifest, icons) → bump `CACHE` in `sw.js` (`hanzi-garden-v5` → `v6`). The service worker serves these cache-first, so without a bump devices keep the old copy. New files needed offline must also go into `CORE`.
...
- **Children and storage keys.** Up to 4 children per device, told apart by animal avatars only (never names). Child `p1` uses the original keys `hanzi-garden-progress-v1` / `hanzi-garden-session-v1`; others add `-p2`…`-p4`. The child list and shared device settings (speech rate, voice) live in `hanzi-garden-kids-v1`. Never rename or rewrite `p1`'s keys — that is what keeps existing progress safe across updates.
- **Release log.** Every release that bumps `CACHE` adds an entry at the top of README.md「更新记录」, named after the cache version (v6, v7, …), written for parents in plain Chinese: what changed for them, not how.
