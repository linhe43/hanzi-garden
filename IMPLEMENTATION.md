...
### Step 12 · 多个孩子共用一台设备（v2 之后追加）

- 家长设置最上面是「谁在学」：用小动物头像（🐰🐻🐼🦊）区分，最多 4 个孩子，**不填名字**。
- 每个孩子独立：星星、花园、每个字的进度、字库、每天新字数、拼音开关、学习时长。语速和声音是设备设置，大家共用。
- 存储：第一个孩子沿用原来的 `hanzi-garden-progress-v1` 和 `hanzi-garden-session-v1`，数据不搬家；其他孩子加后缀 `-p2`…`-p4`；孩子列表和设备设置在 `hanzi-garden-kids-v1`。
- 清空：「只清空当前字库的进度」清掉这个字库的字，以及在这个字库得到的星星（`starsByLib`，从这个版本开始记录；之前的星星无法区分，保留）；「清空这个孩子的全部进度」只清当前孩子；「移除这个孩子」删除他的数据。三个按钮都要按两次确认。
